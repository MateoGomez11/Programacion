...
public class SellerController {
    ...
    public ResponseEntity<ResponseDTO> createSeller(@RequestBody Seller seller) {
        ...
        if (findSeller == null) {
            ...
            return new ResponseEntity<>(new ResponseDTO(409, "Ya existe un vendedor con ese codigo", null), HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<ResponseDTO> getSellerById(@PathVariable String code)
    {
        ...
        if (findSeller == null)
        {
            return new ResponseEntity<>(new ResponseDTO(404,"No existe un vendedor con ese codigo", null), HttpStatus.OK);
        }
        else {
